...
- Definition of addition for the "Chapter 2" natural numbers, `Chapter2.Nat`
- Establishment of basic properties of this addition
...
theorem def_2_2_1_a (m: Nat) : 0 + m = m := prop_2_1_16_a (fun _ sum ↦ sum++) _

theorem def_2_2_1_b (n m: Nat) : n++ + m = (n+m)++ := by rfl

/-- 1 + m = (0++) + m = m++ -/
theorem one_add (m:Nat) : 1 + m = m++ := by
  rw [show 1 = 0++ from rfl, def_2_2_1_b, def_2_2_1_a]

/-- 2 + m = (1++) + m = (m++)++ -/
theorem two_add (m:Nat) : 2 + m = (m++)++ := by
  rw [show 2 = 1++ from rfl, def_2_2_1_b, one_add]
...
theorem exercise_2_2_1 (a b c:Nat) : (a + b) + c = a + (b + c) := by
  sorry

abbrev prop_2_2_5 := exercise_2_2_1

/-- Proposition 2.2.6 (Cancellation law). Let a, b, c be natural numbers such that a + b = a + c. Then we have b = c.-/
theorem prop_2_2_6 (a b c:Nat) (habc: a + b = a + c) : b = c := by
  ...
  . -- First consider the base case $a=0$.  Then we have $0+b=0+c$,
    ...
    rwa [def_2_2_1_a, def_2_2_1_a] at hbc
  ...
  rw [def_2_2_1_b, def_2_2_1_b] at hbc
  -- By Axiom 2.4, we have $a+b=a+c$.
  replace hbc := axiom_2_4_a hbc
...
theorem prop_2_2_8 {a:Nat} (b:Nat) (ha: a.isPos) : (a + b).isPos := by
  ...
  have : (a+b)++ ≠ 0 := axiom_2_3 _
  exact this
  -- This closes the induction.

theorem prop_2_2_8_b {a:Nat} (b:Nat) (ha: a.isPos) : (b + a).isPos := by
  rw [prop_2_2_4]
  exact prop_2_2_8 _ ha

/-- Corollary 2.2.9. If a and b are natural numbers such that a + b = 0, then a = 0 and b = 0.-/
theorem cor_2_2_9 (a b:Nat) (hab: a + b = 0) : a = 0 ∧ b = 0 := by
...
theorem exercise_2_2_3_a (a:Nat) : a ≥ a := by
  sorry

abbrev prop_2_2_12_a := exercise_2_2_3_a

/-- (b) (Order is transitive) If a ≥ b and b ≥ c, then a ≥ c. -/
theorem exercise_2_2_3_b {a b c:Nat} (hab: a ≥ b) (hbc: b ≥ c) : a ≥ c := by
  sorry

theorem prop_2_2_12_b {a b c:Nat} (hab: a ≥ b) (hbc: b ≥ c) : a ≥ c := exercise_2_2_3_b hab hbc

/-- (c) (Order is anti-symmetric) If a ≥ b and b ≥ a, then a = b. -/
theorem exercise_2_2_3_c {a b:Nat} (hab: a ≥ b) (hba: b ≥ a) : a = b := by
  sorry

theorem prop_2_2_12_c {a b:Nat} (hab: a ≥ b) (hba: b ≥ a) : a = b := exercise_2_2_3_c hab hba

/-- (d) (Addition preserves order) a ≥ b if and only if a + c ≥ b + c. -/
theorem exercise_2_2_3_d (a b c:Nat) : a ≥ b ↔ a + c ≥ b + c := by
  sorry

abbrev prop_2_2_12_d := exercise_2_2_3_d

/-- (e) a < b if and only if a++ ≤ b. -/
theorem exercise_2_2_3_e (a b:Nat) : a < b ↔ a++ ≤ b := by
  sorry

abbrev prop_2_2_12_e := exercise_2_2_3_e

/-- (f) a < b if and only if b = a + d for some positive number d. -/
theorem exercise_2_2_3_f (a b:Nat) : a < b ↔ ∃ d:Nat, d.isPos ∧ b = a + d := by
  sorry

abbrev prop_2_2_12_f := exercise_2_2_3_f

/-- If a < b then a ̸= b by definition,-/
theorem prop_2_2_13_a (a b:Nat) : a < b → a ≠ b := by
  intro h; exact h.2

/-- and if a > b then a ̸= b by definition. -/
theorem prop_2_2_13_b (a b:Nat) : a > b → a ≠ b := by
  intro h; exact h.2.symm

/-- If a > b and a < b then by Proposition 2.2.12 we have a = b, a contradiction -/
theorem prop_2_2_13_c (a b:Nat) : a < b ∧ a > b → False := by
  intro h
  have := (prop_2_2_12_c (Nat.le_of_lt h.1) (Nat.le_of_lt h.2)).symm
  have := prop_2_2_13_a _ _ h.1
  contradiction
...
theorem prop_2_2_13 (a b:Nat) : a < b ∨ a = b ∨ a > b := by
...
-/
theorem exercise_2_2_5 {m₀:Nat} {P: Nat → Prop} (hind: ∀ m, m ≥ m₀ → (∀ m', m₀ ≤ m' ∧ m' < m → P m') → P m) : ∀ m, m ≥ m₀ → P m := by
  sorry

theorem proposition_2_2_14 {m₀:Nat} {P: Nat → Prop} (hind: ∀ m, m ≥ m₀ → (∀ m', m₀ ≤ m' ∧ m' < m → P m') → P m) : ∀ m, m ≥ m₀ → P m := exercise_2_2_5 hind

theorem strong_induction {m₀:Nat} {P: Nat → Prop} (hind: ∀ m, m ≥ m₀ → (∀ m', m₀ ≤ m' ∧ m' < m → P m') → P m) : ∀ m, m ≥ m₀ → P m := exercise_2_2_5 hind
...
numbers m ≤ n-/
theorem exercise_2_2_6 {n:Nat} {P: Nat → Prop}  (hind: ∀ m, P (m++) → P m) (hn: P n) : ∀ m, m ≤ n → P m := by
  sorry
...
theorem exercise_2_2_7 {n:Nat} {P: Nat → Prop} (hind: ∀ m, P m → P (m++)) : P n → ∀ m, m ≥ n → P m := by
  sorry
