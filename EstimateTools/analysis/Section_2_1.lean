...
theorem axiom_2_3 (n:Nat) : n++ ≠ 0 := by
  by_contra h
...
theorem axiom_2_4_a {n m:Nat} (hnm: n++ = m++) : n = m := by
  rwa [Nat.succ.injEq] at hnm

theorem axiom_2_4_b (n m:Nat) : n ≠ m → n++ ≠ m++ := by
  intro h
  contrapose! h
  exact axiom_2_4_a h
