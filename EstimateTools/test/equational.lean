...
abbrev S (z:G) (x:G) := (x ◇ z) ◇ z

abbrev f (x y:G) := x ◇ (S y x)

/-  The main equation is $$x=(yx)S_z(x)$$. -/

lemma main_eq (h: Equation1689) (x y z:G) : x = (y ◇ x) ◇ S z x := h x y z
...
```
Take $$b,c,d$$ as in Lemma 2 to rewrite $$af(b,c)=aS_d(a)=f(a,d)$$.  On the other hand, Lemma 1 with $$a=b$$ and $$c$$ replaced by $$d$$ implies $$a^3 = a f(a,d)$$ so overall we get $$a=(af(a,d))f(a,d)$$, which is as desired for $$e=f(a,d)$$. -/

/-
*End of the proof:* For any $$a,y$$, using the notation $$e$$ from Lemma 3, the main equation gives $$a=(ya)S_e(a)=(ya)a=S_a(y)$$. Inserting this back into the main equation gives $$(zy)a=y$$ for any $$a,y,z$$. Thus $$ab=((da)c)b=c$$ for any $$a,b,c,d$$, thus $$a=bc=d$$ for any $$a,b,c,d$$.
````
-/
