...
\begin{lemma}[Log of addition]\label{log-add}\uses{log-order-def}\lean{log_mul, log_mul'}\leanok  For orders of magnitude $\Theta(X), \Theta(Y)$, we have $\log(\Theta(X) + \Theta(Y)) = \max(\log(\Theta(X)), \log(\Theta(Y)))$.
\end{lemma}
